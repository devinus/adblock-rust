...
use crate::lists::{parse_filters, parse_filter, ParsedFilter};
...
impl Engine {
    ...
    fn add_network_filter(&mut self, filter: NetworkFilter) {
        match self.blocker.filter_add(filter) {
            Ok(_) => (),
            Err(BlockerError::BlockerFilterError(e)) => eprintln!("Encountered filter error {:?} when adding", e),
            ...
            Err(e) => eprintln!("Encountered unexpected error {:?} when checking for filter existence", e),
        }
    }
